...
extern void system_call_handler(void) {
	...
	case SYSCALL_SEND: {
		...
		kprints("\n");
		if(RecieverThreadID != -1){ //dvs hvis reciever er klar
			//int RecievePortID = SYSCALL_ARGUMENTS.rdi;
			//kopiere data
			...
			//thread_table[RecieverThreadID].data.registers.integer_registers.rbx = SYSCALL_ARGUMENTS.rbx;
			thread_table[RecieverThreadID].data.registers.integer_registers.rdi = SYSCALL_ARGUMENTS.rdi;
			thread_table[RecieverThreadID].data.registers.integer_registers.rsi = SYSCALL_ARGUMENTS.rsi;
			...
			schedule =1;
			kprints("Send: Reciever IKKE klar\n");
			SYSCALL_ARGUMENTS.rax = ALL_OK;
		}
		kprints("Send slut\n");
		break;
	...
	case SYSCALL_RECEIVE: {
		...
		kprints("\n");
		if (port_table[receiverPortID].sender_queue.head != -1){	// hvis sender-køen IKKE er tom, modtages besked
			...
			thread_table[sendThread].data.registers.integer_registers.rdi = thread_table[sendThread].data.owner;
			...
			if (incomingMsgType != SYSCALL_ARGUMENTS.rsi) {
				...
				//SYSCALL_ARGUMENTS.rbx = thread_table[sendThread].data.registers.integer_registers.rbx;
			...
			thread_queue_enqueue(&ready_queue, sendThread);//release sender
			port_table[receiverPortID].receiver=-1;//sætter reciever til "ingen"
			//thread_queue_dequeue(&port_table[receiverPortID].sender_queue);
